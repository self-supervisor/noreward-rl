...
parser.add_argument(
    "-l", "--log-dir", type=str, default="tmp/doom", help="Log directory path"
)
parser.add_argument(
    "-n",
    "--dry-run",
...
parser.add_argument(
    "--noReward", action="store_true", help="Remove all extrinsic reward"
)
parser.add_argument(
    "--noLifeReward",
    action="store_true",
...
parser.add_argument(
    "--savio", action="store_true", help="Savio or KNL cpu cluster hacks"
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="Checkpoint dir (generally ..../train/) to load from.",
)
...
def new_cmd(session, name, cmd, mode, logdir, shell):
    ...
    if mode == "tmux":
        ...
        return (
            name,
            "nohup {} -c {} >{}/{}.{}.out 2>&1 & echo kill $! >>{}/kill.sh".format(
...
def create_commands(
    ...
    cmds_map = [
        new_cmd(session, "ps", base_cmd + ["--job-name", "ps"], mode, logdir, shell)
    ]
    ...
    if mode == "tmux":
        cmds += [
            ...
            "kill -9 $( lsof -i:{}-{} -t ) > /dev/null 2>&1".format(
                psPort, num_workers + psPort
...
def run():
    ...
    delay = (
        220 * args.expId
        if "doom" in args.env_id.lower() or "labyrinth" in args.env_id.lower()
        else 5 * args.expId
    )
    ...
    if args.dry_run:
        print(
            "Dry-run mode due to -n flag, otherwise the following commands would be executed:"
        )
    else:
