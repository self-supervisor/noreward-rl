...
class FastSaver(tf.train.Saver):
    def save(
        ...
        super(FastSaver, self).save(
            sess, save_path, global_step, latest_filename, meta_graph_suffix, False
        )
...
def run(args, server):
    ...
    if args.pretrain is not None:
        variables_to_restore = [
            v for v in tf.trainable_variables() if not v.name.startswith("local")
        ]
    ...
    def init_fn(ses):
        ...
        if args.pretrain is not None:
            ...
            logger.info(
                "==> Done restoring model! Restored %d variables.",
...
def main(_):
    ...
    parser.add_argument(
        "--psPort", default=12222, type=int, help="Port number for parameter server"
    )
    parser.add_argument(
        "--delay", default=0, type=int, help="delay start by these many seconds"
    )
    parser.add_argument(
        "--pretrain",
        type=str,
