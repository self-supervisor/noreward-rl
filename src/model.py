...
def inverseUniverseHead(x, final_shape, mean_or_sigma="mean", nConvs=4):
    ...
    for i in range(nConvs - 1):
        ...
        deconv_shape2 = deconv_shape2[:-1]
    x = deconv2d(
        x, [bs] + final_shape[1:], "dl4" + mean_or_sigma, [3, 3], [2, 2], prevNumFeat=32
    )
    return x
...
class StatePredictor(object):
    ...
    def __init__(self, ob_space, ac_space, designHead="universe", unsupType="state"):
        ...
        phi1_copy = tf.identity(phi1)
        # forward model: f(phi1,asample) -> phi2
        # Note: no backprop to asample of policy: it is treated as fixed for predictor training

        f_mean = tf.concat(1, [phi1, asample])
        f_mean = tf.nn.relu(
            linear(
                f_mean,
        ...
        f_sigma = tf.concat(1, [phi1_copy, asample])
        f_sigma = tf.nn.relu(
            linear(
                f_sigma,
                phi1.get_shape()[1].value,
        ...
        self.bonus = tf.reduce_mean(self.mse - self.f_sigma, name="bonus")
        self.forwardloss = 0.5 * tf.reduce_mean(
