    ...
    **_
    ...
    if envWrap:
        ...
        env = env_wrapper.BufferedObsEnv(
            env, skip=frame_skip, shape=fshape, maxFrames=False
        )
        if frame_skip > 1:
            env = env_wrapper.SkipEnv(env, skip=frame_skip)
    elif noLifeReward:
...
class DiagnosticsInfoI(vectorized.Filter):
    ...
    def _after_step(self, observation, reward, done, info):
        ...
        if self._local_t % self._log_interval == 0:
            ...
            if info.get("stats.gauges.diagnostics.lag.action") is not None:
                to_log["diagnostics/action_lag_lb"] = info[
                    "stats.gauges.diagnostics.lag.action"
                ][0]
            ...
            if info.get("stats.gauges.diagnostics.clock_skew") is not None:
                to_log["diagnostics/clock_skew_lb"] = info[
                    "stats.gauges.diagnostics.clock_skew"
                ][0]
        ...
        if done:
            logger.info(
                "True Game terminating: env_episode_reward=%s episode_length=%s",
